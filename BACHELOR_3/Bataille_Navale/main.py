...
class BatailleNavale:
    def __init__(self):
        self.TAILLE_GRILLE = 20  # Nouvelle taille de grille
        ...
        # Initialisation des images avant tout
        self.img_eau = ImageTk.PhotoImage(Image.open("images/eau.png").resize((30, 30)))
        self.img_bateau = ImageTk.PhotoImage(Image.open("images/bateau.png").resize((30, 30)))
        self.img_touche = ImageTk.PhotoImage(Image.open("images/touche.png").resize((30, 30)))
        self.img_rate = ImageTk.PhotoImage(Image.open("images/rate.png").resize((30, 30)))
    ...
    def _creer_grille(self, parent: tk.Frame, grille_adverse: bool) -> list[list[tk.Button]]:
        grille = []
        for x in range(self.TAILLE_GRILLE):
            ligne = []
            for y in range(self.TAILLE_GRILLE):
                bouton = tk.Button(parent, 
                image=self.img_eau,  # Image de fond permanente
                compound='center',    # Permet la superposition
                width=30, 
                height=30)
                if grille_adverse:
                    bouton.configure(command=lambda x=x, y=y: self._tour_joueur(x, y))
                else:
                    bouton.configure(command=lambda x=x, y=y: self._placer_navire(x, y))
    ...
    def _tour_joueur(self, x: int, y: int):
        ...
        if touche:
            bouton.configure(image=self.img_touche, compound='center')  # Superposer l'image touche
            if coule:
                messagebox.showinfo("Touché !", "Navire coulé !")
        else:
            bouton.configure(image=self.img_rate, compound='center')    # Superposer l'image raté
    ...
    def _placer_navire(self, x: int, y: int):
        ...
        if self.joueur_humain.plateau.ajouter_navire(navire):
            ...
            for pos_x, pos_y in positions:
                self.boutons_joueur[pos_x][pos_y].configure(bg="gray")  # Utiliser gris au lieu d'une image
